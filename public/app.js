...
// توحيد رموز الرياضيات (x → س ، × ، جذور … إلخ)
function localizeMathSymbols(text) {
  if (!text) return "";
  let t = text;

  // المتغير x إلى س في السياق الرياضي
  t = t
    .replace(/\bx\b/g, "س")
...
// تنظيف نص الإجابة من الرموز الزائدة
function cleanAnswer(text) {
  ...
  // توحيد رموز الرياضيات إلى العربية
  cleaned = localizeMathSymbols(cleaned);

  return cleaned.trim();
}

// تحويل الكسور والأسس إلى HTML مرتب
function fractionsAndPowersToHtml(txt) {
  ...
  t = t.replace(
    /\[\[FRAC:([^|]+)\|([^\]]+)\]\]/g,
    (m, top, bot) =>
      `<span class="frac"><span class="top">${top}</span><span class="bar"></span><span class="bot">${bot}</span></span>`
  );
  t = t.replace(
    /\[\[POW:([^|]+)\|([^\]]+)\]\]/g,
    (m, base, exp) =>
...
if (!elMicBtn && elInput) {
  ...
  elMicBtn.style.cssText =
    "margin-top:8px;padding:6px 12px;border-radius:999px;border:1px solid rgba(56,189,248,0.8);background:#020617;color:#e5e7eb;cursor:pointer;font-size:14px";
  const parent = elInput.parentElement || elForm || document.body;
  parent.appendChild(elMicBtn);
}

// زر تبديل “الإجابة الصوتية” — يكون تحت زر السؤال الصوتي (الستايل يتحكم بالمكان)
...
if (!elTTSBtn) {
  ...
  if (elMicBtn) {
    ...
    (elInput.parentElement || elForm || document.body).appendChild(
      elTTSBtn
    );
  }
}
...
function addMessage(text, who = "assistant") {
  ...
  div.className =
    "message " + (who === "user" ? "user" : "assistant");
...
async function ask() {
  ...
  try {
    ...
    if (!resp) {
      thinking.remove();
      addMessage(
        "⚠ تعذر الاتصال بالخادم. حاولي بعد قليل.",
        "assistant"
      );
      return;
    }
    ...
    if (reply) {
      ...
      // لا نعرض رسالة الخادم التقنية، فقط رسالة لطيفة بالعربي
      addMessage(
        "⚠ تعذّر إكمال الإجابة الآن، حاولي بعد قليل.",
        "assistant"
      );
    } else {
      addMessage(
        "⚠ ما وصلت إجابة مفهومة من الخادم.",
        "assistant"
      );
    }
  } catch (e) {
    thinking.remove();
    addMessage(
      "⚠ صار خطأ بالاتصال، جربي مرة ثانية.",
      "assistant"
    );
  }
}
...
function ensureRecognition() {
  ...
  if (!SR) {
    alert(
      "العفو، المتصفح لا يدعم السؤال الصوتي (جرّبي Google Chrome)."
    );
    return null;
  }
  const rec = new SR();
...
// ==== الإجابة الصوتية: زر صغير + تنظيف للنص المنطوق ====
(function () {
  ...
  function prepareForSpeech(text) {
    ...
    // حذف الروابط والنصوص الإنجليزية الطويلة
    ...
    // ناقص فقط إذا كانت بين أعداد (٣ - ٢)
    t = t.replace(
      /([0-9٠-٩]+)\s*-\s*([0-9٠-٩]+)/g,
      "$1 ناقص $2"
    );
    // أي شرطات أخرى نعتبرها فاصل فقط
    t = t.replace(/-/g, " ");
