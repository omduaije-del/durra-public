...
async function ask() {
  ...
  try {
    ...
    if (!resp) {
      setAnswer("⚠ تعذر الاتصال بالخادم. حاول مرة أخرى بعد قليل.");
      return;
    }
    ...
    if (!answer) {
      if (data.error) {
        answer = "⚠ الخادم قال: " + data.error;
      } else {
        answer = "⚠ ما وصلت إجابة مفهومة من الخادم.";
      }
    }
...
function ensureRecognition() {
  ...
  rec.onerror = (e) => {
    ...
    alert("⚠ تعذر الاستماع للسؤال. حاول مرة أخرى.");
  };
